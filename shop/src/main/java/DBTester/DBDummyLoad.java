...
public class DBDummyLoad {


    private static final String JDBC_URL = "jdbc:postgresql://localhost/postgre_db";
    private static final String JDBC_USERNAME = "ishop";
    private static final String JDBC_PASSWORD = "admin";


    private static final String IMG_PATH = "shop/src/main/java/DBTester/test-data";
    private static final String MEDIA_PATH = "shop/src/main/webapp/static/media/";
    ...
    public DBDummyLoad() throws SQLException {
    ...
    public static void main(String[] args) throws Exception {
        ...
        props.setProperty("user", JDBC_USERNAME);
...
//        props.setProperty("ssl","true");
        ...
        conn.setAutoCommit(false);
            clearDb(conn);
            Map<String, Integer> producerIdMap = insertProducers(conn, producers);
            Map<String, Integer> categoryIdMap = insertCategories(conn, categories);
            insertProducts(conn, categories, categoryIdMap, producerIdMap);
        conn.commit();
    ...
    private static void clearDb(Connection c) throws SQLException {
        try (Statement st = c.createStatement()) {
            ...
            st.executeUpdate("delete from brand");
        }
    ...
    private static Map<String, Integer> insertProducers(Connection c, List<Producer> producers) throws Exception {
        ...
        int i = 1;
        try (PreparedStatement ps = c.prepareStatement("insert into brand values (?,?,?)")) {
    ...
    private static String getProductImageLink(Category category) throws IOException {
        ...
        return "static/media/" + productImageName;
    }
    ...
    private static String generateProductDescription(String categoryName) {
        ...
        try {
            String[] desc = {};
            String[] options = {};
            Field fieldDesc = DBDummyLoad.class.getDeclaredField(staticFieldDescName);
            desc = (String[]) fieldDesc.get(DBDummyLoad.class);
            try {
                Field fieldDescOptions = DBDummyLoad.class.getDeclaredField(staticFieldDescOptionsName);
                options = (String[]) fieldDescOptions.get(DBDummyLoad.class);
    ...
    private static class Category implements Comparable<Category> {
        ...
        private List<Producer> createProducers() {
            ...
            try {
                Field field = DBDummyLoad.class.getDeclaredField(staticFieldName);
                String[] data = (String[]) field.get(DBDummyLoad.class);
